...
def search_reports(query: dict) -> dict:
    ...
    q = {k: _norm(v) for k, v in query.items() if v}
    ...
    if hard:
        matches = [r for r in approved if any(_norm(r.get(k)) == v for k, v in hard.items())]
        if not matches:
            return {"mode": "none", "results": []}
        return {"mode": "detail", "results": [masked_report(r) for r in matches]}

    name_qs = {k: q[k] for k in NAME_FIELDS if q.get(k)}
    if not name_qs:
        return {"mode": "none", "results": []}

    matches = [r for r in approved if any(_norm(r.get(k)) == v for k, v in name_qs.items())]
    if not matches:
        return {"mode": "none", "results": []}
    candidates = [{
        "id": r["id"],
    ...
    return {"mode": "candidates", "results": candidates}
...
def smart_search(value: str) -> dict:
    ...
    matches = [r for r in approved if any(_norm(r.get(f)) == v for f in HARD_FIELDS)]
    if matches:
        return {"mode": "detail", "results": [masked_report(r) for r in matches]}

    matches = [r for r in approved if any(_norm(r.get(f)) == v for f in NAME_FIELDS)]
    if not matches:
        return {"mode": "none", "results": []}
    candidates = [{
        "id": r["id"],
